<#
the goal is to find the only two numbers in each row where one evenly divides the other - that is, where the result of the division operation is a whole number.
...
foreach ($line in $arr) {
    ...
    for ($i = 0; $i -lt $row.Count; $i++) {
        for ($j = 0; $j -lt $row.Count; $j++) {
            ...
            $first = [int]::Parse($row[$i])
            $second = [int]::Parse($row[$j])
            if ($first % $second -ne 0) {
                continue
            }
